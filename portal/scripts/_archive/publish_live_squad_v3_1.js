...
// --- AGENTE: CARLOS COPY & FELIPE FOTO (Sincronia Total v3.1.2) ---
const post = {
  id: 'b8c9d0e1-f2a3-4b5c-6d7e-8f9a0b1c2d3e',
  titulo: 'Baterias de Estado Sólido: A Revolução das 1000 Milhas em 2026',
  categoria: 'Energia',
  autor: 'Conselho Editorial FolhaByte',
  imagem_url: 'https://images.unsplash.com/photo-1558444479-c8f027d6a524', // Curadoria v4: Abstract Battery Energy
...
[IMAGEM: https://images.unsplash.com/photo-1620712943543-bcc4628c6757 | LEGENDA: Infraestrutura global: Estações Megawatt integradas a redes inteligentes de energia renovável.]

## A Fronteira da Eletrônica de Consumo

Para o usuário comum, a mudança é sentida na palma da mão. Os novos smartphones e wearables agora possuem autonomias de sete dias em uso intenso. O conceito de "carregar o celular à noite" tornou-se obsoleto, substituído por transações de energia de curta duração via indução ambiental nas casas e escritórios inteligentes.
...
[IMAGEM: https://images.unsplash.com/photo-1550751827-4bd374c3f58b | LEGENDA: Integração Invisível: Hardware de próxima geração onde a energia é onipresente e sem fios.]
...
async function orchestrate() {
  ...
  // Extrai apenas as URLs dos blocos de imagem no corpo do artigo
  const bodyImages = (post.conteudo_markdown.match(/\[IMAGEM: (https?:\/\/[^\s|]+)/g) || [])
    .map(m => m.replace('[IMAGEM: ', '').trim());
  const allImages = [post.imagem_url, ...bodyImages];
  const uniqueImages = new Set(allImages);

  if (uniqueImages.size < allImages.length) {
    console.error("❌ Rebeca: Erro Crítico! Repetição visual detectada. Abortando deploy.");
    process.exit(1);
  }
  console.log(`✅ Rebeca: ${allImages.length} imagens únicas validadas. Sincronia semântica confirmada.`);
